...
"""Base class for substeps in the xnn step."""
...
class Substep(seamm.Node):
    """A base class for substeps in the xnn subflowchart.

    The top-level Xnn step iterates over the substeps
    of its subflowchart and calls each substep's `run()` method. Each substep
    is responsible for preparing its own input, invoking xnn,
    ...
    * `version` / `git_revision` - delegated to the plug-in package so that
    ...
    @property
    def version(self):
        """The semantic version of the xnn_step package."""
        return xnn_step.__version__

    @property
    def git_revision(self):
        """The git revision of the xnn_step package."""
    ...
    @property
    def options(self):
        """The xnn options, delegated to the parent step."""
        return self.parent.options

    @property
    def input_only(self):
        """Whether to write the input only, not run xnn."""
